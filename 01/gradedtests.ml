...
let part2_tests : suite = [
  ...
  GradedTest ("Problem2-1Manual", 3, [
    ("third_of_three1", assert_eqf (fun () -> third_of_three another_triple) "hello world");
    ("third_of_three2", assert_eqf (fun () -> third_of_three (5,10,15)) 15);
    ("third_of_three3", assert_eqf (fun () -> third_of_three ((),"b",true)) true);
  ]);
...
let part3_tests : suite = [
  ...
  GradedTest ("Problem3-4Manual", 5, [
    ("rev_t2", assert_eqf (fun () -> rev_t [27; 88]) [88; 27]);
    ("rev_t3", assert_eqf (fun () -> rev_t [1;2]) [2;1]);
    ("rev_t4", assert_eqf (fun () -> rev_t ["hello";"world"]) ["world";"hello"]);
  ]);
...
let part4_tests : suite = [
  ...
  GradedTest ("Problem4-3harder", 5, [
    ("interpret1", assert_eqf (fun () -> interpret ctxt_test e1) 6L);
    ("interpret2", assert_eqf (fun () -> interpret ctxt_test e2) 5L);
    ("interpret3", (fun () -> try ignore (interpret ctxt_test e3); failwith "bad interpret" with Not_found -> ()));
  ]);
  ...
  GradedTest ("Problem4-4harder", 5, [
    ("optimize4", assert_eqf (fun () -> optimize (Add(Const 31L, Const 14L))) (Const 45L));
    ("optimize5", assert_eqf (fun () -> optimize (Mult(Const 0L, Var "x"))) (Const 0L));
    ("optimize6", assert_eqf (fun () -> optimize (Add(Const 10L, Mult(Const 0L, Var "x")))) (Const 10L));
  ]);

  GradedTest ("Problem4-4hardest", 5, [
    ("optimize7", assert_eqf (fun () -> optimize (Mult(Add(Const 1L, Mult(Var "x", Const 0L)), Var "y"))) (Var "y"));
    ("optimize8", assert_eqf (fun () -> optimize (Add(Const 15L, Mult(Const 0L, Var "x")))) (Const 15L));
    ("optimize9", assert_eqf (fun () -> optimize (Mult(Var "x", Neg(Const 0L)))) (Const 0L));
  ]);
