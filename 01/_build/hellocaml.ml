...
(*
  ...
  send mail to cis341@seas.upenn.edu or drop by office hours.  Learning OCaml
...
(*
  ...
  Follow the instructions at http://www.cis.upenn.edu/~cis341/toolchain.ml
  ...
  includes suitable targets for building, testing, cleaning, and zipping up your
...
let another_triple : int * bool * string = (10, false, "hello world")
...
let rev_t (l: 'a list) : 'a list =
  let rec helper (acc: 'a list) (l: 'a list): 'a list =
    begin match l with
      | []    -> acc
...
(*Insert an element to a sorted list, if exist, make no change*)
(*
  Time: O(n)
  Space: O(1)
*)
let rec insert (x:'a) (l:'a list) : 'a list =
  begin match l with
  | []    -> [x] 
  | h::tl -> if x < h then x::h::tl else (if x = h then h::tl else h::(insert x tl))
  end 
...
let rec lookup (x:string) (c:ctxt) : int64 =
  begin match c with
  | []                -> raise Not_found
  | (str, intger)::tl -> if x = str then intger else lookup x tl
  end
  (* compare str1 str2 == 0 *)
